...
public class Uniform<T> implements Selection<T>{
   ...
   */
  protected int choose_one( unalcol.integer.Uniform g ){
    return g.next();
  }
  ...
  public int choose_one( Tagged<T>[] x ){
    return choose_one(new unalcol.integer.Uniform(x.length));
  }
  ...
  public int[] apply( int n, Tagged<T>[] x){
	  unalcol.integer.Uniform g =  new unalcol.integer.Uniform(x.length);
    return g.generate(n);
  }
}