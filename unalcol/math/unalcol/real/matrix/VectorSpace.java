...
import unalcol.real.matrix.Init;
...
public class VectorSpace implements unalcol.math.algebra.VectorSpace<double[][]> {
    @Override
    public double[][] identity( double[][] x ){
        int n = Util.rows(x);
        int m = Util.columns(x);
        return Init.zero(n,m);
    }
